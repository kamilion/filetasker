...
parse_to_epoch_from_date_generic()
{
  ...
  debug_out " Parsed Filedate: ${file_datestamp} - Date: ${file_year}-${file_month}-${file_day} Time: ${file_hours}:${file_mins}:${file_secs} Zulu - Epoch: @${file_epoch} or ${file_timestamp}"
}
...
parse_to_epoch_from_yyyy_mm_dd_dir()
{
  ...
  file_timestamp=`date -u -d @${file_epoch}`
  debug_out " Parsed Dirdate: ${my_date} - Date: ${my_year}-${my_month}-${my_day} - Epoch: @${file_epoch} or ${file_timestamp}"
}
...
parse_to_epoch_from_yyyymmdd_dir()
{
  ...
  file_epoch=`date +%s -d $my_date`
  file_timestamp=`date -u -d @${file_epoch}`
  debug_out " Parsed Dirdate: ${my_date} - Epoch: @${file_epoch} or ${file_timestamp}"
}
...
generate_yyyy_mm_dd_date_dir_from_epoch()
{
  ...
  debug_out " Generated Date Directory from: ${my_epoch} to: ${date_dir}"
}
